...
void MKS_DLP::moveMax_Z()
{
	...
	do_blocking_move_to_z(destination[Z_AXIS],2*PRINT_FEEDRATE);	//�ƶ�Z�ᣬ
	stepper.synchronize();
	report_current_position();
...
/*----------�ж��ļ���ʽ-----------*/
	...
	if(!strstr((char *)&head.identifier[0],"MKSDLP"))
		{
		card.printingHasFinished();		//��MKSDLP�ļ���ʽ������ӡ
		return;
		}
...
void MKS_DLP::getHead2()
{
	...
	//���ڼ���cbddlp������һ�����ص�ת��
	head.LedOnTime_bak=head.LedOnTime;//�ع�ʱ��
	head.BottomLedOnTime_bak=head.BottomLedOnTime;	//�ײ��ع�ʱ��
}
void MKS_DLP::startFileprint()
{
	cbddlp_file_header.antiAliasingLevel=0;
    mksdlp.ssd.init();//���³�ʼ��2k�������ⳤʱ������֮����2k���޷�������ӡ��
	getHead1();
	getHead2();
...
uint8_t MKS_DLP::bank2disp_enable(uint8_t bank_used_id,uint8_t scan_en,uint8_t rd_sdram_en)
{
	...
	//cmd.data2 =  (bank_used_id<<4 | 0x80)&0xF0;		//��������SDRAM
...
uint8_t MKS_DLP:: reTransmission()
{
	for(uint8_t i=0;i<MAX_RETRAN_TIMES;i++)
		...
		CPLD_SPI_CS_OP = 0;
		HAL_SPI_Transmit_DMA(&hspin, &bmp.line.d_frame_bakup[0], TXDATA_LEN);	  //����DMA����
...
/*******************************************************************************
��������1
flag =0 ,����ǰ��ҳ,��Χ(0~line-1)
flag = 1,��������ҳ,��Χ(line~2560-1)
*******************************************************************************/
...
void MKS_DLP:: line_fill_zero(uint16_t line,uint8_t bank_used_id)
{
	...
	uint8_t cur_block;
		...
		bmp.line.d_frame.mark1 = 0x40 | (bank_used_id<<4) | ((cur_line >> 8)&0x0F);//tan ---дsdram
		bmp.line.d_frame.mark2 =  cur_line&0xFF;
		memset(bmp.line.d_frame.data,0,DATA_LEN);       //tan ---��������ȫΪ0
		bmp.line.d_frame.CRC16 = CRC16_XMODEM(&bmp.line.d_frame.mark1,DATA_CRC_LEN);
...
void MKS_DLP::line_gen_data(uint16_t line,uint8_t bank_used_id)
{
	...
	bmp.line.d_frame.mark1 = 0x40 | (bank_used_id<<4) | ((line >> 8)&0x0F);//tan ---дsdram
	bmp.line.d_frame.mark2 =  line&0xFF;
	memset(bmp.line.d_frame.data,0,DATA_LEN);

	p=bmp.line.d_frame.data;
	q = Line_Pixel;         //tan ---����Ƭ�У��ѽ���������һ������(2k����ʾ������)
	for(i=0;i<DATA_LEN;i++) //tan ---�����ݴ�Line_Pixel���Ƶ�bmp.line.d_frame.data
	{
		for(j=0;j<8;j++)
		{
			*p <<= 1;     //tan ---bmp.line.d_frame.data����һλ��ѭ��8�Σ���Line_Pixel[]������ת��
			if(*q++ == 1)//tan ---��Line_Pixel���ֽ���ת��Ϊ��λ���ŵ�bmp.line.d_frame.data
				*p |= 1;
		}
		p++;
	}
	...
	bmp.line.crc_status = CPLD_CRC_IP;              //tan ---����crc�ܽţ��ߵ�ƽУ��������
	if(bmp.line.crc_status == 0)	//�ش��ж�
	{
		reTransmission();
...
void MKS_DLP::get_sdcard_bmps()
{
	...
	rc = card.gets(sd_char,4);
	//bmp.totalPoint = sd_char[0]<<8 | sd_char[1];	//�ܵ���
	bmp.totalPoint = sd_char[0]<<24 | sd_char[1]<<16 | sd_char[2]<<8 | sd_char[3];	//�ܵ���
	...
	for(int i=0;i<bmp.totalPoint;i++)
	{
		...
		bmp.line.start_py = sd_char[0]<<8 | sd_char[1];	//��ʼ��
		if(bmp.line.start_py > Y_RATIO-1) bmp.line.start_py = Y_RATIO-1;

		rc = card.gets(sd_char,2);
		bmp.line.end_py = sd_char[0]<<8 | sd_char[1];	//������
		if(bmp.line.end_py > Y_RATIO-1) bmp.line.end_py = Y_RATIO-1;

		rc = card.gets(sd_char,2);
		bmp.coordinate_X = sd_char[0]<<8 | sd_char[1];	//�к�
		if(bmp.coordinate_X > X_RATIO-1) bmp.coordinate_X = X_RATIO-1;

		if(i==0)// tan ---��һ�п�ʼ֮ǰ��ǰ������ȫ����0
			{
					//ǰ��������"0"
				line_fill_all_zero(0,bmp.coordinate_X,WORK_USED_BANK);
				bmp.current_line = 	bmp.coordinate_X;	//tan ---�е������ݵĿ�ʼ�У��˴�Ϊ����2k������ʼ��
			}

		if(bmp.current_line == bmp.coordinate_X)    //tan ---����Ƭ������ͼƬ������Ҫ��������ת��1
			{
			p = &Line_Pixel[bmp.line.start_py];     //tan ---ÿһ�е����ݣ�(����Ϊ:һ���ֽ�Ϊ2k����һ����)������ʼ�㵽�����㣬��д1�������Ϳ���ת��2k������ʾ����
			for(int j = bmp.line.start_py;j<bmp.line.end_py+1;j++)
				{
					if(j<Y_RATIO/2)	{*p = 1;}           //tan ---ǰ��ҳ��������
					else			{*(p+FILLCODE) = 1;}//tan ---����ҳ��������(����������48λ)
					p++;
				}
			}
		else
			{
			line_gen_data(bmp.current_line,WORK_USED_BANK);			//һ������//tan ---����һ�����ݸ�ʽ�������͵�sdram��
			line_gen_data_TFT(bmp.current_line);
/*----------------------------------------*/
			while((bmp.coordinate_X - bmp.current_line) > 1)        //tan ---Ϊʲô�ڴ���һ��֮�󣬵ڶ���Ҫ��0��������Ϊ�м��п�϶�ġ�
			{
				bmp.current_line++;
...
/*----------------------------------------*/
			...
			memset(Line_Pixel,0,sizeof(Line_Pixel));//tan ---��������
			p = &Line_Pixel[bmp.line.start_py];       //tan ---����Ƭ���ݽ�����Line_Pixel
			for(int j = bmp.line.start_py;j<bmp.line.end_py+1;j++)
			{
					if(j<Y_RATIO/2)	{*p = 1;}
					else			{*(p+FILLCODE) = 1;}
		...
		if(bmp.totalPoint>1)
			{
			line_gen_data(bmp.current_line,WORK_USED_BANK);		//����һ��
			line_gen_data_TFT(bmp.current_line);
			}
		while(hdma_spin_tx.State == HAL_DMA_STATE_BUSY);				//�ȴ�DMA��������
		CPLD_SPI_CS_OP = 1;

		line_fill_all_zero(1,bmp.current_line+1,WORK_USED_BANK);		//����������"0"
...
void MKS_DLP::bottomLayerPrint()
{
	...
	if(status.curBottomlayers == 0) //��ȡ�ײ�ͼƬ
		{
		...
		if(mksdlp.print_file_type==1)
		{
			...
			bank2disp_enable(WORK_USED_BANK,1,1);	//��ʾͼƬ
		}
		else
		{
			if(cbddlp_file_header.antiAliasingLevel>1)
		...
		do_blocking_move_to_z(destination[Z_AXIS],PRINT_FEEDRATE);	//�ƶ�Z�ᣬ
		stepper.synchronize();
		report_current_position();
		...
		||(status.exposure == false && aalay_cnt>0 && cbddlp_file_header.antiAliasingLevel>1))
	{
		if(mksdlp.print_file_type!=1)
		{
			...
			if(cbddlp_file_header.antiAliasingLevel>1)
			{
				...
				bank2disp_enable(aalay_cnt,1,1);	//��ʾͼƬ
				aalay_cnt++;
				status.exposure = true;
				//led_on();				//���ع�
				clr_ticket();
		...
		if(status.curBottomlayers < head.Bottomlayers)
		{
			...
			if(cbddlp_file_header.antiAliasingLevel>1)
			{
				if(aalay_cnt>=cbddlp_file_header.antiAliasingLevel)
				{
					...
					moveRelative_Z(home_bump_mm_P[2],PRINT_FEEDRATE);		//�ƶ�Z�ᣬ
					if(mksReprint.mks_printer_state == MKS_WORKING)
						moveRelative_Z(head.thickness-home_bump_mm_P[2],PRINT_FEEDRATE);		//�ƶ�Z�ᣬ
...
void MKS_DLP::GeneralLayersPrint()
{
			...
			||(((status.currentLayer-1) < head.totalLayers-head.Bottomlayers)&&(mksdlp.print_file_type!=1)))
		{
			if(cbddlp_file_header.antiAliasingLevel>1)
			{
				...
				if(aalay_cnt==0)
				{
					...
					moveRelativeZandGetBmp(home_bump_mm_P[2],PRINT_FEEDRATE);//�ƶ�Z�ᣬ����ȡͼƬ
		...
		||(status.exposure == false && aalay_cnt>0 &&cbddlp_file_header.antiAliasingLevel>1))
	{
		...
		if(cbddlp_file_header.antiAliasingLevel>1)
		{
			bank2disp_enable(aalay_cnt-1,1,1);	//��ʾͼƬ
			if(aalay_cnt==1)
			{
				led_on();			//���ع�
			}
...
*/
	...
	//if((status.currentLayer > head.totalLayers) || card_eof)	//��ӡ����
	if((((status.currentLayer > head.totalLayers)||card_eof)&&(mksdlp.print_file_type==1))
		||((status.currentLayer-1 > head.totalLayers)&&(mksdlp.print_file_type!=1)))	//��ӡ����
		{
		printingHasFinished();
		return;
		}
...
void MKS_DLP::ExposureDataTrans_circle()
{
	...
	uint8_t *p;
		...
		line_fill_all_zero(1,exposure_bmp_data_circle[CIRCLE_BMP_DATA_ROW-1][2]+1,TEST_USED_BANK);	//����������"0"
...
void MKS_DLP::ExposureDataTrans_square()
{
	...
	uint8_t *p;
		...
		line_fill_all_zero(1,exposure_bmp_data_square[SQUARE_BMP_DATA_ROW-1][2]+1,TEST_USED_BANK);	//����������"0"
#if ENABLED(MKS_DLP_WRITE_FILE)
				close_file();
#endif
...
void MKS_DLP::ExposureTest()
{
	...
	grag_test();
		...
		LED_BACK_LIGHT_OP = 0;
		status.led_status = false;
		bank2disp_enable(CLEAN_USED_BANK,1,1);	//������Ļ
		ssd.sleep_in();
		HAL_Delay(20);
		bank2disp_enable(CLEAN_USED_BANK,0,0);	//�ر�RGB�ź�
...
{
	if(status.currentLayer) return;	//�Ѿ���ӡ���ײ㣬����������
...
uint32_t MKS_DLP::get_totalPrintTime()
{
	...
	uint32_t totalLedOnTime;		//����ʱ��
	uint32_t totalLedOffTime;		//����ʱ��
	uint32_t totalMoveTime;			//�ƶ�ʱ��
	uint32_t totalWastageTime;		//���㣬����������ʱ��
...
{
	uint32_t curLedOnTime;		//����ʱ��
	uint32_t curLedOffTime;		//����ʱ��
	uint32_t curMoveTime;		//�ƶ�ʱ��
...
{
	// ��ӡһ��ʱ�����ò�ͼ���������� bmp_layer_buf�� ͼ��Ϊ�ڰ�ɫ��0:�ڣ�1:��
	// �� bmp_layer_buf ������ʾ��TFT�趨������
	// return 1 ʱ��������д���ɣ���Ҫ������ʾ����
	if(displayInTFT_ON)
		{
		displayInTFT_ON = 0;
...
void MKS_DLP::write_Information_paused()
{
	...
	epr_write_data(EPR_SAV_SDPOS, (uint8_t *)&mksReprint.sdpos,sizeof(mksReprint.sdpos)); //��ǰ�ع�������ʼλ��
	epr_write_data(EPR_SAV_C_LAYER, (uint8_t *)&cLayer,sizeof(cLayer)); 					//��ǰ�ع���
...
void MKS_DLP::write_Information_layer(DLP_MOVE_STATUS status)
{
	...
	if(status == DLP_STOPED)
		...
		epr_write_data(poweroff_resume.addr+7,(uint8_t *)&cseconds ,4); //ʱ��
		}
...
uint16_t MKS_DLP::Can_Resume_Print()
{
	...
	if(poweroff_resume.status[max_i] == DLP_STOPED)
		{
		mksReprint.sdpos = poweroff_resume.sdpos[max_i];	//��ȡ�ļ�ƫ��
		//�ָ���ӡʱ��
...
void MKS_DLP::get_cbddlp_file_header()
{
		...
		if(cbddlp_file_header.antiAliasingLevel>4)//Ŀǰcbddlp��ʽ��mks��ֻ������4���Ҷ�
...
void get_pic_display(uint8_t sel)
{
	...
	if(sel==1)
	{
		...
		ii=(float)w/(float)m;//�����м���
		jj=(float)h/(float)n;//�����м���
